...
public class ShoppingWork {
	...
	public static ShoppingWork from(ShoppingWorkExpenses expenses, ShoppingWorkTaxExpenses taxExpenses, ShoppingCouponPrice couponPrice) {
		...
		if(expenses.getValue() == null) {
			return new ShoppingWork(SisyutuKingaku.ZERO, couponPrice);
		}
		...
		if(couponPrice.getValue() == null) {
			return new ShoppingWork(SisyutuKingaku.from(workValue), ShoppingCouponPrice.from(null));
		}
		...
		if (compareToValue > 0) {
			return new ShoppingWork(SisyutuKingaku.ZERO, ShoppingCouponPrice.from(discountValue));
		}
		// 割引後の金額が0)：仕事金額は割引適応でなし。残クーポン値もなし
		if (compareToValue == 0) {
			return new ShoppingWork(SisyutuKingaku.ZERO, ShoppingCouponPrice.from(null));
		}
		// 割引後の金額が0より大きい)：仕事金額は割引適応後の値。残クーポン値はなし
		return new ShoppingWork(SisyutuKingaku.from(discountValue), ShoppingCouponPrice.from(null));

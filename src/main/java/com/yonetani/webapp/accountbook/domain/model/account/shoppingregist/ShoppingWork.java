...
public class ShoppingWork {
	 ...
	 *
	 */
	public static ShoppingWork from(ShoppingWorkExpenses expenses, ShoppingWorkTaxExpenses taxExpenses, CouponAmount couponPrice) {
		// ガード節(「仕事金額」項目がnull)
		if(expenses == null) {
		...
		if(couponPrice.getValue() == null) {
			return new ShoppingWork(ExpenditureAmount.from(workValue), CouponAmount.ZERO);
		}
		...
		if (compareToValue > 0) {
			return new ShoppingWork(ExpenditureAmount.ZERO, CouponAmount.from(discountValue));
		}
		// 割引後の金額が0)：仕事金額は割引適応でなし。残クーポン値もなし
		if (compareToValue == 0) {
			return new ShoppingWork(ExpenditureAmount.ZERO, CouponAmount.ZERO);
		}
		// 割引後の金額が0より大きい)：仕事金額は割引適応後の値。残クーポン値はなし
		return new ShoppingWork(ExpenditureAmount.from(discountValue), CouponAmount.ZERO);

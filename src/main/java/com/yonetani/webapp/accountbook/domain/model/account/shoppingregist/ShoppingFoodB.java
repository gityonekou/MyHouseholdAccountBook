/**
 * 「食料品B(無駄遣い)」項目のドメインです。
 * 「食料品B(無駄遣い)金額」項目と「消費税:食料品B(無駄遣い)金額」項目の加算値から「クーポン」項目を適応(減算)した値が「食料品B(無駄遣い)」項目の値となります。
 *
 *------------------------------------------------
 * 更新履歴
 * 日付       : version  コメントなど
...
public class ShoppingFoodB {
	...
	/**
	 *<pre>
	 * 「食料品B(無駄遣い)」項目のドメインを生成します。
	 * 「食料品B(無駄遣い)金額」項目と「消費税:食料品B(無駄遣い)金額」項目の加算値から「クーポン」項目を適応(減算)した値となります。
	 * 　クーポン適応により、金額が0円となる場合は食料品B(無駄遣い)金額にnullが設定されます。
	 * 
	 * [非ガード節]
	 * ・食料品B(無駄遣い)金額がnull値
	 * ・クーポン金額がnull値
	 ...
	 * @return 「食料品B(無駄遣い)」項目ドメイン
	 *
	 */
	public static ShoppingFoodB from(ShoppingFoodBExpenses expenses, ShoppingFoodBTaxExpenses taxExpenses, ShoppingCouponPrice couponPrice) {
		...
		if(expenses.getValue() == null) {
			return new ShoppingFoodB(ExpenditureAmount.ZERO, couponPrice);
		}
		...
		if(couponPrice.getValue() == null) {
			return new ShoppingFoodB(ExpenditureAmount.from(foodValue), ShoppingCouponPrice.from(null));
		}

		// 食料品B(無駄遣い)金額からクーポン金額を割引
		BigDecimal discountValue = foodValue.add(couponPrice.getValue());

		// 割引後の金額がマイナス値)：食料品B(無駄遣い)金額は割引適応でなし。残クーポン値は食料品B(無駄遣い)金額の値(マイナス値)
		int compareToValue = BigDecimal.ZERO.compareTo(discountValue);
		if (compareToValue > 0) {
			return new ShoppingFoodB(ExpenditureAmount.ZERO, ShoppingCouponPrice.from(discountValue));
		}
		// 割引後の金額が0)：食料品B(無駄遣い)金額は割引適応でなし。残クーポン値もなし
		if (compareToValue == 0) {
			return new ShoppingFoodB(ExpenditureAmount.ZERO, ShoppingCouponPrice.from(null));
		}
		// 割引後の金額が0より大きい)：食料品B(無駄遣い)金額は割引適応後の値。残クーポン値はなし
		return new ShoppingFoodB(ExpenditureAmount.from(discountValue), ShoppingCouponPrice.from(null));
		
	}
	
	/**
	 *<pre>
	 * 「食料品B(無駄遣い)」項目の値があるかどうかを判定します。

/**
 * 「衣料品(私服)」項目のドメインです。
 * 「衣料品(私服)金額」項目と「消費税:衣料品(私服)金額」項目の加算値から「クーポン」項目を適応(減算)した値が「衣料品(私服)」項目の値となります。
 *
 *------------------------------------------------
 * 更新履歴
 * 日付       : version  コメントなど
...
public class ShoppingClothes {
	...
	/**
	 *<pre>
	 * 「衣料品(私服)」項目のドメインを生成します。
	 * 「衣料品(私服)金額」項目と「消費税:衣料品(私服)金額」項目の加算値から「クーポン」項目を適応(減算)した値となります。
	 * 　クーポン適応により、金額が0円となる場合は衣料品(私服)金額にnullが設定されます。
	 * 
	 * [非ガード節]
	 * ・衣料品(私服)金額がnull値
	 * ・クーポン金額がnull値
	...
	public static ShoppingClothes from(ShoppingClothesExpenses expenses, ShoppingClothesTaxExpenses taxExpenses, ShoppingCouponPrice couponPrice) {
		...
		// 衣料品(私服)金額を計算
		BigDecimal clothesValue = (taxExpenses.getValue() == null) ? expenses.getValue() : expenses.getValue().add(taxExpenses.getValue());
		...
		BigDecimal discountValue = clothesValue.add(couponPrice.getValue());

...
@Getter
@EqualsAndHashCode
public class ExpenditureItemCode {
	 ...
	 *
	 */
	public static ExpenditureItemCode from(String expenditureItemCode) {
		// ガード節(空文字列)
		if(!StringUtils.hasLength(expenditureItemCode)) {
			throw new MyHouseholdAccountBookRuntimeException("「支出項目コード」項目の設定値が空文字列です。管理者に問い合わせてください。");
		}
		// ガード節(長さが4桁でない)
		if(expenditureItemCode.length() != 4) {
			throw new MyHouseholdAccountBookRuntimeException("「支出項目コード」項目の設定値が不正です。管理者に問い合わせてください。[expenditureItemCode=" + expenditureItemCode + "]");
		}
		// ガード節(数値に変換できない(数値4桁:0パディング))
		try {
			Integer.parseInt(expenditureItemCode);
		} catch(NumberFormatException ex) {
			throw new MyHouseholdAccountBookRuntimeException("「支出項目コード」項目の設定値が不正です。管理者に問い合わせてください。[expenditureItemCode=" + expenditureItemCode + "]");
		}

		return new ExpenditureItemCode(expenditureItemCode);
	}

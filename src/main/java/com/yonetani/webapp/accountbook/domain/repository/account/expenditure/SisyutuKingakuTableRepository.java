...
public interface SisyutuKingakuTableRepository {
	 ...
	 *
	 */
	int add(ExpenditureAmountItem data);
	 ...
	 *
	 */
	int update(ExpenditureAmountItem data);
	
	/**
	 *<pre>
	 * 支出金額テーブル：SISYUTU_KINGAKU_TABLEの主キー（ユーザID、支出項目コード）で支出金額テーブルを検索し、結果を取得します。
	 *</pre>
	 * @param search 検索条件(ユーザID, 年月,支出項目コード)
	 * @return 支出金額情報
	 *
	 */
	ExpenditureAmountItem findByPrimaryKey(SearchQueryUserIdAndYearMonthAndExpenditureItemCode search);
	 ...
	 *
	 */
	ExpenditureAmountItemInquiryList findBy(SearchQueryUserIdAndYearMonth search);

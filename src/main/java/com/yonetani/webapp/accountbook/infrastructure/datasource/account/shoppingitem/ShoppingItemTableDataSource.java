...
public class ShoppingItemTableDataSource implements ShoppingItemTableRepository {
	...
	public ShoppingItemInquiryList findByIdAndSisyutuItemCode(SearchQueryUserIdAndSisyutuItemCode search) {
		// 検索結果を取得
		List<ShoppingItemReadWriteDto> searchResult = mapper.findByIdAndSisyutuItemCode(UserIdAndSisyutuItemCodeSearchQueryDto.from(
		...
		if(searchResult == null) {
			...
			// 検索結果ありの場合、ドメインに変換して返却
			return ShoppingItemInquiryList.from(searchResult.stream().map(dto -> createShoppingItem(dto))

...
public class ShoppingRegistExpenditureAndSisyutuKingakuComponent {
	...
	private final ExpenditureItemCode dineOutItemCode = ExpenditureItemCode.from(MyHouseholdAccountBookContent.SISYUTU_ITEM_CODE_GAISYOKU_VALUE);
	// 定数:日用消耗品の支出項目コード
	private final ExpenditureItemCode consumerGoodsItemCode = ExpenditureItemCode.from(MyHouseholdAccountBookContent.SISYUTU_ITEM_CODE_NITIYOU_SYOUMOUHIN_VALUE);
	// 定数:被服費の支出項目コード
	private final ExpenditureItemCode clothesItemCode = ExpenditureItemCode.from(MyHouseholdAccountBookContent.SISYUTU_ITEM_CODE_HIFUKU_VALUE);
	// 定数:仕事(流動経費)の支出項目コード
	private final ExpenditureItemCode workItemCode = ExpenditureItemCode.from(MyHouseholdAccountBookContent.SISYUTU_ITEM_CODE_RYUUDOU_KEIHI_VALUE);
	// 定数:住居設備の支出項目コード
	private final ExpenditureItemCode houseEquipmentItemCode = ExpenditureItemCode.from(MyHouseholdAccountBookContent.SISYUTU_ITEM_CODE_JYUUKYO_SETUBI_VALUE);
	...
	public List<String> checkExpenditureRegistItemList(List<ExpenditureRegistItem> checkList) {
		...
		checkList.forEach(data -> {
			StringBuilder key = new StringBuilder(data.getExpenditureItemCode());
			if(StringUtils.hasLength(data.getExpenditureCategory())) {
				key.append(data.getExpenditureCategory());
			} else {
		...
		// 飲食(無駄遣いなし)
		if(checkMap.get(foodItemCode.getValue() + ExpenditureCategory.NON_WASTED.getValue()) == null) {
			// エラーメッセージを追加
			responseMessage.add("飲食(無駄づかいなし)に対応する支出情報が登録されていないか複数登録されています。支出登録画面で飲食(無駄使いなし)の支出情報を登録してから再度実行してください。");
		}
		// 飲食(無駄遣いB)
		if(checkMap.get(foodItemCode.getValue() + ExpenditureCategory.WASTED_B.getValue()) == null) {
			// エラーメッセージを追加
			responseMessage.add("飲食(無駄遣いB)に対応する支出情報が登録されていないか複数登録されています。支出登録画面で飲食(無駄遣いB)の支出情報を登録してから再度実行してください。");
		}
		// 飲食(無駄使いC)
		if(checkMap.get(foodItemCode.getValue() + ExpenditureCategory.WASTED_C.getValue()) == null) {
			// エラーメッセージを追加
			responseMessage.add("飲食(無駄使いC)に対応する支出情報が登録されていないか複数登録されています。支出登録画面で飲食(無駄使いC)の支出情報を登録してから再度実行してください。");
		}
		// 一人プチ贅沢・外食
		if(checkMap.get(dineOutItemCode.getValue() + ExpenditureCategory.NON_WASTED.getValue()) == null) {
			// エラーメッセージを追加
			responseMessage.add("一人プチ贅沢・外食(無駄づかいなし)に対応する支出情報が登録されていないか複数登録されています。支出登録画面で一人プチ贅沢・外食(無駄づかいなし)の支出情報を登録してから再度実行してください。");
		}
		// 日用消耗品
		if(checkMap.get(consumerGoodsItemCode.getValue() + ExpenditureCategory.NON_WASTED.getValue()) == null) {
			// エラーメッセージを追加
			responseMessage.add("日用消耗品(無駄づかいなし)に対応する支出情報が登録されていないか複数登録されています。支出登録画面で日用消耗品(無駄づかいなし)の支出情報を登録してから再度実行してください。");
		}
		// 被服費
		if(checkMap.get(clothesItemCode.getValue() + ExpenditureCategory.NON_WASTED.getValue()) == null) {
			// エラーメッセージを追加
			responseMessage.add("被服費(無駄づかいなし)に対応する支出情報が登録されていないか複数登録されています。支出登録画面で被服費(無駄づかいなし)の支出情報を登録してから再度実行してください。");
		}
		// 仕事(流動経費)
		if(checkMap.get(workItemCode.getValue() + ExpenditureCategory.NON_WASTED.getValue()) == null) {
			// エラーメッセージを追加
			responseMessage.add("仕事(流動経費)(無駄づかいなし)に対応する支出情報が登録されていないか複数登録されています。支出登録画面で仕事(流動経費)(無駄づかいなし)の支出情報を登録してから再度実行してください。");
		}
		// 住居設備
		if(checkMap.get(houseEquipmentItemCode.getValue() + ExpenditureCategory.NON_WASTED.getValue()) == null) {
			// エラーメッセージを追加

...
class IncomeAndExpenditureInitIntegrationTest {
    ...
    /**
     * 正常系テスト: 必須登録データ未登録時のメッセージ確認
     *
     * <p>買い物登録に必須の支出情報が登録されていない場合、
     * 警告メッセージが8件返却されることを確認する。</p>
     */
    @Test
    @Sql(scripts = {
        "/sql/initsql/schema_test.sql",
    ...
    /**
     * 正常系テスト: 全必須登録データ登録済み時のメッセージ確認（0件）
     *
     * <p>買い物登録に必須の支出情報が全て固定費に登録されている場合、
     * 警告メッセージが0件となることを確認する。</p>
     */
    @Test
    @Sql(scripts = {
        "/sql/initsql/schema_test.sql",
    ...
    /**
     * 正常系テスト: 一部必須登録データのみ登録時のメッセージ確認（4件）
     *
     * <p>買い物登録に必須の支出情報が一部のみ固定費に登録されている場合、
     * 未登録の項目分だけ警告メッセージが表示されることを確認する。</p>
     */
    @Test
    @Sql(scripts = {
        "/sql/initsql/schema_test.sql",

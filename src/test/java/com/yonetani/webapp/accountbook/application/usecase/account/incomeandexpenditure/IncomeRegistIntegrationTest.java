...
class IncomeRegistIntegrationTest {
    ...
    /**
     *<pre>
     * テスト①：正常系：新規追加画面表示_フォーム初期値確認
     *
     * 【検証内容】
     * ・アクションが新規追加(add)であること
     * ・収入区分名が「【新規追加】」であること
     * ・収入コード・収入区分・詳細・金額がnullであること
     * ・画面表示用収入一覧が2件設定されること
     *</pre>
     */
    @Test
    ...
    /**
     *<pre>
     * テスト②：正常系：更新画面表示_選択収入情報のフォーム設定確認
     *
     * 【検証内容】
     * ・セッションに存在する収入コード(20260125120000001)の情報がフォームに設定されること
     * ・アクションが更新(update)であること
     * ・収入コード・収入区分名（【給料】）・収入区分('1')・詳細・金額(300000)が正しく設定されること
     *</pre>
     */
    @Test
    ...
    /**
     *<pre>
     * テスト③：異常系：更新画面表示_収入コード不存在エラー
     *
     * 【検証内容】
     * ・セッションに存在しない収入コードを指定した場合、MyHouseholdAccountBookRuntimeExceptionがスローされること
     * ・例外メッセージに「更新対象の収入情報がセッションに存在しません」が含まれること
     * ・例外メッセージに指定した収入コード値が含まれること
     *</pre>
     */
    @Test
     ...
     * ・アクション・収入区分名・収入区分・詳細・金額（負の値）が変更されないこと
     * ・画面表示用収入一覧が2件設定されること
     *</pre>
     */
    @Test
    ...
    /**
     *<pre>
     * テスト⑤：正常系：収入新規追加_セッションへの追加確認
     *
     * 【検証内容】
     * ・セッションの収入リストに1件追加されること（元のサイズ＋1）
     * ・追加された収入のデータタイプが新規(new)、アクションが追加(add)であること
     * ・収入コードがyyyyMMddHHmmssSSS形式の17桁自動生成コードであること
     * ・収入区分('3':積立からの取崩し)・詳細・金額が正しく設定されること
     * ・完了メッセージに「仮登録しました」・収入区分名(積立からの取崩し)・金額(50,000円)が含まれること
     *</pre>
     */
    @Test
    ...
    /**
     *<pre>
     * テスト⑥：正常系：収入更新_セッションの更新確認
     *
     * 【検証内容】
     * ・収入リストのサイズが変わらないこと
     * ・指定した収入コードの収入区分（'1'→'2'）・詳細・金額が更新されること
     * ・完了メッセージに「仮更新しました」・更新後収入区分名(副業)・金額(350,000円)が含まれること
     *</pre>
     */
    @Test
    ...
    /**
     *<pre>
     * テスト⑦：正常系：収入削除_新規データタイプ_セッションから完全削除
     *
     * 【検証内容】
     * ・新規データタイプの収入を削除した場合、セッションリストから完全に削除されること（サイズ-1）
     * ・削除対象の収入コードがリストに存在しないこと
     * ・完了メッセージに「仮削除しました」・収入区分名(給料)が含まれること
     *</pre>
     */
    @Test
    ...
    /**
     *<pre>
     * テスト⑧：正常系：収入削除_ロードデータタイプ_削除アクション設定
     *
     * 【検証内容】
     * ・ロードデータタイプの収入を削除した場合、セッションリストのサイズが変わらないこと
     * ・削除対象の収入がアクション=削除(delete)・データタイプ=ロード(load)でリストに残ること
    ...
    /**
     *<pre>
     * テスト⑨：異常系：収入更新_収入コード不存在エラー
     *
     * 【検証内容】
     * ・セッションに存在しない収入コードで更新しようとした場合、MyHouseholdAccountBookRuntimeExceptionがスローされること
     * ・例外メッセージに「更新対象の収入情報がセッションに存在しません」が含まれること
     * ・例外メッセージに指定した収入コード値が含まれること
     *</pre>
     */
    ...
    /**
     *<pre>
     * テスト⑩：異常系：収入削除_収入コード不存在エラー
     *
     * 【検証内容】
     * ・セッションに存在しない収入コードで削除しようとした場合、MyHouseholdAccountBookRuntimeExceptionがスローされること
     * ・例外メッセージに「更新対象の収入情報がセッションに存在しません」が含まれること
     * ・例外メッセージに指定した収入コード値が含まれること
     *</pre>
     */

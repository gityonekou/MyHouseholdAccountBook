/**
 * TotalPurchasePrice(購入金額合計)のテストクラスです。
 *
 *------------------------------------------------
 * 更新履歴
 ...
 *
 */
@DisplayName("購入金額合計(TotalPurchasePrice)のテスト")
class TotalPurchasePriceTest {

	@Test
	@DisplayName("正常系：正の金額で生成できる")
	void testFrom_正常系_正の金額() {
		// 実行
		TotalPurchasePrice amount = TotalPurchasePrice.from(new BigDecimal("10000.00"));
	...
	void testFrom_異常系_null値() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			...
			() -> TotalPurchasePrice.from(null)
		);
		assertTrue(exception.getMessage().contains("購入金額合計"));
	...
	void testFrom_異常系_負の金額() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> TotalPurchasePrice.from(new BigDecimal("-1000.00"))
		);
		assertTrue(exception.getMessage().contains("購入金額合計"));
	...
	void testFrom_異常系_スケール値不正() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> TotalPurchasePrice.from(new BigDecimal("10000"))
		);
		assertTrue(exception.getMessage().contains("購入金額合計"));
	...
	void testAdd_正常系() {
		// 準備
		TotalPurchasePrice amount1 = TotalPurchasePrice.from(new BigDecimal("10000.00"));
		TotalPurchasePrice amount2 = TotalPurchasePrice.from(new BigDecimal("5000.00"));

		// 実行
		TotalPurchasePrice result = amount1.add(amount2);
	...
	void testEquals_正常系_値ベースで一致() {
		// 準備
		TotalPurchasePrice amount1 = TotalPurchasePrice.from(new BigDecimal("10000.00"));
		TotalPurchasePrice amount2 = TotalPurchasePrice.from(new BigDecimal("10000.00"));

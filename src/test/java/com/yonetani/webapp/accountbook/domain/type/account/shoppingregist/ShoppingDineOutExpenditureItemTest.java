/**
 * ShoppingDineOutExpenditureItem(外食)のテストクラスです。
 *
 *------------------------------------------------
 * 更新履歴
 ...
 *
 */
@DisplayName("外食(ShoppingDineOutExpenditureItem)のテスト")
class ShoppingDineOutExpenditureItemTest {
	...
	void testFrom_Success() {
		...
		// 実行
		ShoppingDineOutExpenditureItem result = ShoppingDineOutExpenditureItem.from(expenditureAmount, taxExpenses);

		// 検証
		assertNotNull(result);
		assertEquals(new BigDecimal("10000.00"), result.getValue()); // 購入金額 + 消費税
		assertEquals(expenditureAmount, result.getShoppingDineOutExpenditureAmount());
		assertEquals(taxExpenses, result.getShoppingDineOutTaxExpenses());
	...
	void testNULL() {
		// 準備
		ShoppingDineOutExpenditureItem nullValue = ShoppingDineOutExpenditureItem.from(ShoppingExpenditureAmount.NULL, ShoppingTaxExpenses.NULL);

		// 検証
		assertNotNull(ShoppingDineOutExpenditureItem.NULL);
		assertNull(ShoppingDineOutExpenditureItem.NULL.getValue());
		assertEquals(ShoppingDineOutExpenditureItem.NULL, nullValue);
	}
	...
	void testFrom_MinorNull() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			...
			() -> ShoppingDineOutExpenditureItem.from(null, taxExpenses));

		// エラーメッセージ検証
		assertTrue(exception.getMessage().contains("外食購入金額"));
	...
	void testFrom_SevereNull() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> ShoppingDineOutExpenditureItem.from(expenditureAmount, null));

		// エラーメッセージ検証
		assertTrue(exception.getMessage().contains("外食消費税"));
		assertTrue(exception.getMessage().contains("null"));
	}
	...
	void testFrom_ExpenditureNullValue_TaxHasValue() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> ShoppingDineOutExpenditureItem.from(expenditureAmount, taxExpenses));

		// エラーメッセージ検証
		assertTrue(exception.getMessage().contains("「外食」項目"));
	...
	@Test
	@DisplayName("正常系：add(ShoppingDineOutExpenditureItem)メソッドで加算")
	void testAdd_Success() {
		// 準備
		ShoppingDineOutExpenditureItem base = ShoppingDineOutExpenditureItem.from(
			ShoppingExpenditureAmount.from(new BigDecimal("2000.00")),
			ShoppingTaxExpenses.from(new BigDecimal("3000.00"))
		);
		ShoppingDineOutExpenditureItem addValue = ShoppingDineOutExpenditureItem.from(
			ShoppingExpenditureAmount.from(new BigDecimal("1000.00")),
			ShoppingTaxExpenses.from(new BigDecimal("2000.00"))
		);

		// 実行
		ShoppingDineOutExpenditureItem result = base.add(addValue);

		// 検証
		assertNotNull(result);
		assertEquals(0, new BigDecimal("8000.00").compareTo(result.getValue()));
		assertEquals(0, new BigDecimal("3000.00").compareTo(result.getShoppingDineOutExpenditureAmount().getValue()));
		assertEquals(0, new BigDecimal("5000.00").compareTo(result.getShoppingDineOutTaxExpenses().getValue()));
	}

	@Test
	@DisplayName("正常系：add(ShoppingDineOutExpenditureItem)でnullオブジェクトを加算")
	void testAdd_NullObject() {
		...
		// 実行
		ShoppingDineOutExpenditureItem result = base.add(null);
	...
	void testAdd_WithNullValue() {
		...
		ShoppingDineOutExpenditureItem nullBase = ShoppingDineOutExpenditureItem.from(
		...
		ShoppingDineOutExpenditureItem addNullValue = ShoppingDineOutExpenditureItem.from(
			ShoppingExpenditureAmount.NULL,
			ShoppingTaxExpenses.NULL
		);

		// 実行
		ShoppingDineOutExpenditureItem result1 = base.add(addNullValue);
		ShoppingDineOutExpenditureItem result2 = nullBase.add(addValue);
		ShoppingDineOutExpenditureItem result3 = nullBase.add(addNullValue);
		...
		assertTrue(result3.getShoppingDineOutExpenditureAmount().isNull());
		assertTrue(result3.getShoppingDineOutTaxExpenses().isNull());
	}
	...
	void testEquals() {
		// 準備
		ShoppingDineOutExpenditureItem value1 = ShoppingDineOutExpenditureItem.from(
			ShoppingExpenditureAmount.from(new BigDecimal("5000.00")),
			ShoppingTaxExpenses.from(new BigDecimal("5000.00"))
		);
		ShoppingDineOutExpenditureItem value2 = ShoppingDineOutExpenditureItem.from(
			ShoppingExpenditureAmount.from(new BigDecimal("5000.00")),
			ShoppingTaxExpenses.from(new BigDecimal("5000.00"))
		);
		ShoppingDineOutExpenditureItem value3 = ShoppingDineOutExpenditureItem.from(
	...
	@Test
	@DisplayName("正常系：applyCoupon(CouponAmount)で外食金額が0円の場合は支出金額0円・クーポンはそのまま残る")
	void testApplyCoupon_ItemZero() {
		...
		// 実行
		ShoppingDineOutItemExpenditureAmount result = base.applyCoupon(coupon);
	...
	@Test
	@DisplayName("正常系：applyCoupon(CouponAmount)でクーポンが0円の場合は割引なしで外食金額がそのまま支出金額となる")
	void testApplyCoupon_CouponZero() {
		...
		// 実行
		ShoppingDineOutItemExpenditureAmount result = base.applyCoupon(CouponAmount.ZERO);
	...
	@Test
	@DisplayName("正常系：applyCoupon(CouponAmount)でクーポン金額が外食金額を上回る場合は支出金額0円・差額が残クーポン額となる")
	...
	@Test
	@DisplayName("正常系：applyCoupon(CouponAmount)でクーポン金額が外食金額とちょうど同額の場合は支出金額0円・残クーポン額もなし")
	...
	@Test
	@DisplayName("正常系：applyCoupon(CouponAmount)でクーポン金額が外食金額を下回る場合は割引後の金額が支出金額となり残クーポン額はなし")

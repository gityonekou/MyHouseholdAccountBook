/**
 * ShoppingConsumerGoodsExpenditureItem(日用品)のテストクラスです。
 *
 *------------------------------------------------
 * 更新履歴
 ...
 *
 */
@DisplayName("日用品(ShoppingConsumerGoodsExpenditureItem)のテスト")
class ShoppingConsumerGoodsExpenditureItemTest {
	...
	void testFrom_Success() {
		...
		// 実行
		ShoppingConsumerGoodsExpenditureItem result = ShoppingConsumerGoodsExpenditureItem.from(expenditureAmount, taxExpenses);

		// 検証
		assertNotNull(result);
		assertEquals(new BigDecimal("10000.00"), result.getValue()); // 購入金額 + 消費税
		assertEquals(expenditureAmount, result.getShoppingConsumerGoodsExpenditureAmount());
		assertEquals(taxExpenses, result.getShoppingConsumerGoodsTaxExpenses());
	...
	void testNULL() {
		// 準備
		ShoppingConsumerGoodsExpenditureItem nullValue = ShoppingConsumerGoodsExpenditureItem.from(ShoppingExpenditureAmount.NULL, ShoppingTaxExpenses.NULL);

		// 検証
		assertNotNull(ShoppingConsumerGoodsExpenditureItem.NULL);
		assertNull(ShoppingConsumerGoodsExpenditureItem.NULL.getValue());
		assertEquals(ShoppingConsumerGoodsExpenditureItem.NULL, nullValue);
	}
	...
	void testFrom_MinorNull() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			...
			() -> ShoppingConsumerGoodsExpenditureItem.from(null, taxExpenses));

		// エラーメッセージ検証
		assertTrue(exception.getMessage().contains("日用品購入金額"));
	...
	void testFrom_SevereNull() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> ShoppingConsumerGoodsExpenditureItem.from(expenditureAmount, null));

		// エラーメッセージ検証
		assertTrue(exception.getMessage().contains("日用品消費税"));
		assertTrue(exception.getMessage().contains("null"));
	}
	...
	void testFrom_ExpenditureNullValue_TaxHasValue() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> ShoppingConsumerGoodsExpenditureItem.from(expenditureAmount, taxExpenses));

		// エラーメッセージ検証
		assertTrue(exception.getMessage().contains("「日用品」項目"));
	...
	@Test
	@DisplayName("正常系：add(ShoppingConsumerGoodsExpenditureItem)メソッドで加算")
	void testAdd_Success() {
		// 準備
		ShoppingConsumerGoodsExpenditureItem base = ShoppingConsumerGoodsExpenditureItem.from(
			ShoppingExpenditureAmount.from(new BigDecimal("2000.00")),
			ShoppingTaxExpenses.from(new BigDecimal("3000.00"))
		);
		ShoppingConsumerGoodsExpenditureItem addValue = ShoppingConsumerGoodsExpenditureItem.from(
			ShoppingExpenditureAmount.from(new BigDecimal("1000.00")),
			ShoppingTaxExpenses.from(new BigDecimal("2000.00"))
		);

		// 実行
		ShoppingConsumerGoodsExpenditureItem result = base.add(addValue);

		// 検証
		assertNotNull(result);
		assertEquals(0, new BigDecimal("8000.00").compareTo(result.getValue()));
		assertEquals(0, new BigDecimal("3000.00").compareTo(result.getShoppingConsumerGoodsExpenditureAmount().getValue()));
		assertEquals(0, new BigDecimal("5000.00").compareTo(result.getShoppingConsumerGoodsTaxExpenses().getValue()));
	}

	@Test
	@DisplayName("正常系：add(ShoppingConsumerGoodsExpenditureItem)でnullオブジェクトを加算")
	void testAdd_NullObject() {
		...
		// 実行
		ShoppingConsumerGoodsExpenditureItem result = base.add(null);
	...
	void testAdd_WithNullValue() {
		...
		ShoppingConsumerGoodsExpenditureItem nullBase = ShoppingConsumerGoodsExpenditureItem.from(
		...
		ShoppingConsumerGoodsExpenditureItem addNullValue = ShoppingConsumerGoodsExpenditureItem.from(
			ShoppingExpenditureAmount.NULL,
			ShoppingTaxExpenses.NULL
		);

		// 実行
		ShoppingConsumerGoodsExpenditureItem result1 = base.add(addNullValue);
		ShoppingConsumerGoodsExpenditureItem result2 = nullBase.add(addValue);
		ShoppingConsumerGoodsExpenditureItem result3 = nullBase.add(addNullValue);
		...
		assertTrue(result3.getShoppingConsumerGoodsExpenditureAmount().isNull());
		assertTrue(result3.getShoppingConsumerGoodsTaxExpenses().isNull());
	}
	...
	void testEquals() {
		// 準備
		ShoppingConsumerGoodsExpenditureItem value1 = ShoppingConsumerGoodsExpenditureItem.from(
			ShoppingExpenditureAmount.from(new BigDecimal("5000.00")),
			ShoppingTaxExpenses.from(new BigDecimal("5000.00"))
		);
		ShoppingConsumerGoodsExpenditureItem value2 = ShoppingConsumerGoodsExpenditureItem.from(
			ShoppingExpenditureAmount.from(new BigDecimal("5000.00")),
			ShoppingTaxExpenses.from(new BigDecimal("5000.00"))
		);
		ShoppingConsumerGoodsExpenditureItem value3 = ShoppingConsumerGoodsExpenditureItem.from(
	...
	@Test
	@DisplayName("正常系：applyCoupon(CouponAmount)で日用品金額が0円の場合は支出金額0円・クーポンはそのまま残る")
	void testApplyCoupon_ItemZero() {
		...
		// 実行
		ShoppingConsumerGoodsItemExpenditureAmount result = base.applyCoupon(coupon);
	...
	@Test
	@DisplayName("正常系：applyCoupon(CouponAmount)でクーポンが0円の場合は割引なしで日用品金額がそのまま支出金額となる")
	void testApplyCoupon_CouponZero() {
		...
		// 実行
		ShoppingConsumerGoodsItemExpenditureAmount result = base.applyCoupon(CouponAmount.ZERO);
	...
	@Test
	@DisplayName("正常系：applyCoupon(CouponAmount)でクーポン金額が日用品金額を上回る場合は支出金額0円・差額が残クーポン額となる")
	...
	@Test
	@DisplayName("正常系：applyCoupon(CouponAmount)でクーポン金額が日用品金額とちょうど同額の場合は支出金額0円・残クーポン額もなし")
	...
	@Test
	@DisplayName("正常系：applyCoupon(CouponAmount)でクーポン金額が日用品金額を下回る場合は割引後の金額が支出金額となり残クーポン額はなし")

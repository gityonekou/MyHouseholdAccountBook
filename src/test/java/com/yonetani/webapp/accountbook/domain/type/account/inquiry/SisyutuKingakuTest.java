 ...
 *
 */
@DisplayName("支出金額(SisyutuKingaku)のテスト")
class SisyutuKingakuTest {

	@Test
	@DisplayName("正常系：正の金額で生成できる")
	void testFrom_正常系_正の金額() {
		// 実行
		SisyutuKingaku amount = SisyutuKingaku.from(new BigDecimal("10000.00"));
	...
	void testFrom_正常系_0円() {
		// 実行
		SisyutuKingaku amount = SisyutuKingaku.from(BigDecimal.ZERO.setScale(2));
	...
	void testZERO定数() {
		// 検証
		assertNotNull(SisyutuKingaku.ZERO);
		assertTrue(SisyutuKingaku.ZERO.isZero());
		assertEquals(BigDecimal.ZERO.setScale(2), SisyutuKingaku.ZERO.getValue());
	}
	...
	void testFrom_異常系_負の金額() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> SisyutuKingaku.from(new BigDecimal("-1000.00"))
		);
		assertTrue(exception.getMessage().contains("支出金額"));
	...
	void testFrom_異常系_スケール値不正() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> SisyutuKingaku.from(new BigDecimal("10000"))
		);
		assertTrue(exception.getMessage().contains("支出金額"));
	...
	void testAdd_正常系() {
		// 準備
		SisyutuKingaku amount1 = SisyutuKingaku.from(new BigDecimal("10000.00"));
		SisyutuKingaku amount2 = SisyutuKingaku.from(new BigDecimal("5000.00"));

		// 実行
		SisyutuKingaku result = amount1.add(amount2);
	...
	void testSubtract_正常系() {
		// 準備
		SisyutuKingaku amount1 = SisyutuKingaku.from(new BigDecimal("10000.00"));
		SisyutuKingaku amount2 = SisyutuKingaku.from(new BigDecimal("3000.00"));

		// 実行
		SisyutuKingaku result = amount1.subtract(amount2);
	...
	void testCompareTo_正常系_等しい() {
		// 準備
		SisyutuKingaku amount1 = SisyutuKingaku.from(new BigDecimal("10000.00"));
		SisyutuKingaku amount2 = SisyutuKingaku.from(new BigDecimal("10000.00"));
	...
	void testCompareTo_正常系_小さい() {
		// 準備
		SisyutuKingaku amount1 = SisyutuKingaku.from(new BigDecimal("5000.00"));
		SisyutuKingaku amount2 = SisyutuKingaku.from(new BigDecimal("10000.00"));
	...
	void testIsZero() {
		// 検証
		assertTrue(SisyutuKingaku.ZERO.isZero());
		assertFalse(SisyutuKingaku.from(new BigDecimal("1.00")).isZero());
	}
	...
	void testIsPositive() {
		// 検証
		assertFalse(SisyutuKingaku.ZERO.isPositive());
		assertTrue(SisyutuKingaku.from(new BigDecimal("1.00")).isPositive());
	}

	@Test
	@DisplayName("正常系：isNegativeが正しく動作する（支出金額は常にfalse）")
	void testIsNegative() {
		// 検証（支出金額は負の値を持てないため、常にfalse）
		assertFalse(SisyutuKingaku.ZERO.isNegative());
		assertFalse(SisyutuKingaku.from(new BigDecimal("1.00")).isNegative());
	}
	...
	void testEquals() {
		...
		SisyutuKingaku amount1 = SisyutuKingaku.from(new BigDecimal("10000.00"));
		SisyutuKingaku amount2 = SisyutuKingaku.from(new BigDecimal("10000.00"));
		SisyutuKingaku amount3 = SisyutuKingaku.from(new BigDecimal("5000.00"));
	...
	void testToString() {
		// 準備
		SisyutuKingaku amount = SisyutuKingaku.from(new BigDecimal("12345.67"));
	...
	void testToFormatString() {
		// 準備
		SisyutuKingaku amount1 = SisyutuKingaku.from(new BigDecimal("12345.67"));
		SisyutuKingaku amount2 = SisyutuKingaku.from(new BigDecimal("1000000.00"));
	...
	void testToIntegerValue() {
		...
		SisyutuKingaku amount1 = SisyutuKingaku.from(new BigDecimal("12345.67"));
		SisyutuKingaku amount2 = SisyutuKingaku.from(new BigDecimal("1000000.00"));
		SisyutuKingaku amount3 = SisyutuKingaku.from(new BigDecimal("12345.44"));
		SisyutuKingaku amount4 = SisyutuKingaku.from(new BigDecimal("12345.50"));

...
class SisyutuKingakuCTest {
	...
	void testFrom_正常系_0円() {
		// 実行
		SisyutuKingakuC amount = SisyutuKingakuC.from(BigDecimal.ZERO.setScale(2));
	...
	void testCompareTo_正常系_等しい() {
		// 準備
		SisyutuKingakuC amount1 = SisyutuKingakuC.from(new BigDecimal("10000.00"));
		SisyutuKingakuC amount2 = SisyutuKingakuC.from(new BigDecimal("10000.00"));
	...
	void testCompareTo_正常系_小さい() {
		// 準備
		SisyutuKingakuC amount1 = SisyutuKingakuC.from(new BigDecimal("5000.00"));
		SisyutuKingakuC amount2 = SisyutuKingakuC.from(new BigDecimal("10000.00"));
	...
	void testIsZero() {
		// 検証
		assertTrue(SisyutuKingakuC.ZERO.isZero());
		assertFalse(SisyutuKingakuC.from(new BigDecimal("1.00")).isZero());
		// null値の場合もisZero()がtrueを返す（NullableMoneyの仕様）
		assertTrue(SisyutuKingakuC.from(null).isZero());
	}
	...
	void testIsNull() {
		// 検証
		assertTrue(SisyutuKingakuC.from(null).isNull());
		assertFalse(SisyutuKingakuC.ZERO.isNull());
		assertFalse(SisyutuKingakuC.from(new BigDecimal("1.00")).isNull());
	}
	...
	void testIsPositive() {
		// 検証
		assertFalse(SisyutuKingakuC.ZERO.isPositive());
		assertTrue(SisyutuKingakuC.from(new BigDecimal("1.00")).isPositive());
		assertFalse(SisyutuKingakuC.from(null).isPositive());
	}

	@Test
	@DisplayName("正常系：isNegativeが正しく動作する（支出金額Cは常にfalse）")
	void testIsNegative() {
		// 検証（支出金額Cは負の値を持てないため、常にfalse）
		assertFalse(SisyutuKingakuC.ZERO.isNegative());
		assertFalse(SisyutuKingakuC.from(new BigDecimal("1.00")).isNegative());
		assertFalse(SisyutuKingakuC.from(null).isNegative());
	}
	...
	void testEquals() {
		...
		SisyutuKingakuC amount1 = SisyutuKingakuC.from(new BigDecimal("10000.00"));
		SisyutuKingakuC amount2 = SisyutuKingakuC.from(new BigDecimal("10000.00"));
		SisyutuKingakuC amount3 = SisyutuKingakuC.from(new BigDecimal("5000.00"));
		SisyutuKingakuC amount4 = SisyutuKingakuC.from(null);
		SisyutuKingakuC amount5 = SisyutuKingakuC.from(null);
	...
	void testToString() {
		// 準備
		SisyutuKingakuC amount1 = SisyutuKingakuC.from(new BigDecimal("12345.67"));
		SisyutuKingakuC amount2 = SisyutuKingakuC.from(null);
	...
	void testToFormatString() {
		// 準備
		SisyutuKingakuC amount1 = SisyutuKingakuC.from(new BigDecimal("12345.67"));
		SisyutuKingakuC amount2 = SisyutuKingakuC.from(new BigDecimal("1000000.00"));
		SisyutuKingakuC amount3 = SisyutuKingakuC.from(null);

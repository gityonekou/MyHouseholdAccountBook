/**
 * BalanceTotalAmount(収支金額合計)のテストクラスです。
 *
 *------------------------------------------------
 * 更新履歴
 * 日付       : version  ブランチ            コメントなど
 * 2026/01/02 : 1.00.00  feature-1.00-dev00  新規作成
 *
 */
package com.yonetani.webapp.accountbook.domain.type.account.incomeandexpenditure;

import static org.junit.jupiter.api.Assertions.*;

import java.math.BigDecimal;

import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;

import com.yonetani.webapp.accountbook.common.exception.MyHouseholdAccountBookRuntimeException;
import com.yonetani.webapp.accountbook.domain.type.common.BalanceAmount;

/**
 *<pre>
 * BalanceTotalAmount(収支金額合計)のテストクラスです。
 *
 *</pre>
 *
 * @author ：Kouki Yonetani
 * @since 家計簿アプリ(1.00)
 *
 */
@DisplayName("収支金額合計(BalanceTotalAmount)のテスト")
class BalanceTotalAmountTest {

	@Test
	@DisplayName("正常系：正の金額で生成できる")
	void testFrom_正常系_正の金額() {
		// 実行
		BalanceTotalAmount amount = BalanceTotalAmount.from(new BigDecimal("10000.00"));

		// 検証
		assertNotNull(amount);
		assertEquals(new BigDecimal("10000.00"), amount.getValue());
		assertEquals("10000.00", amount.toString());
		assertEquals("10,000円", amount.toFormatString());
	}

	@Test
	@DisplayName("正常系：負の金額で生成できる（赤字）")
	void testFrom_正常系_負の金額() {
		// 実行
		BalanceTotalAmount amount = BalanceTotalAmount.from(new BigDecimal("-5000.00"));

		// 検証
		assertNotNull(amount);
		assertEquals(new BigDecimal("-5000.00"), amount.getValue());
		assertTrue(amount.isNegative());
		assertTrue(amount.isDeficit());
	}

	@Test
	@DisplayName("正常系：ZERO定数が使用できる")
	void testZERO定数() {
		// 検証
		assertNotNull(BalanceTotalAmount.ZERO);
		assertTrue(BalanceTotalAmount.ZERO.isZero());
		assertEquals(BigDecimal.ZERO.setScale(2), BalanceTotalAmount.ZERO.getValue());
	}

	@Test
	@DisplayName("異常系：null値で例外が発生する")
	void testFrom_異常系_null値() {
		// 実行 & 検証
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> BalanceTotalAmount.from(null)
		);
		assertTrue(exception.getMessage().contains("収支金額合計"));
		assertTrue(exception.getMessage().contains("null"));
	}

	@Test
	@DisplayName("異常系：スケール値が2以外で例外が発生する")
	void testFrom_異常系_スケール値不正() {
		// 実行 & 検証
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> BalanceTotalAmount.from(new BigDecimal("10000"))
		);
		assertTrue(exception.getMessage().contains("収支金額合計"));
		assertTrue(exception.getMessage().contains("スケール"));
	}

	@Test
	@DisplayName("正常系：加算が正しく動作する")
	void testCalculate_正常系() {
		// 準備
		BalanceTotalAmount amount1 = BalanceTotalAmount.from(new BigDecimal("10000.00"));
		BalanceAmount amount2 = BalanceAmount.from(new BigDecimal("5000.00"));

		// 実行
		BalanceTotalAmount result = amount1.add(amount2);

		// 検証
		assertEquals(new BigDecimal("15000.00"), result.getValue());
		// 元のオブジェクトは変更されていないことを確認（不変性）
		assertEquals(new BigDecimal("10000.00"), amount1.getValue());
		assertEquals(new BigDecimal("5000.00"), amount2.getValue());
	}

	@Test
	@DisplayName("正常系：0への加算が正しく動作する")
	void testAdd_正常系_0への加算() {
		// 準備
		BalanceTotalAmount amount1 = BalanceTotalAmount.ZERO;
		BalanceAmount amount2 = BalanceAmount.from(new BigDecimal("5000.00"));

		// 実行
		BalanceTotalAmount result = amount1.add(amount2);

		// 検証
		assertEquals(new BigDecimal("5000.00"), result.getValue());
		// 元のオブジェクトは変更されていないことを確認（不変性）
		assertEquals(new BigDecimal("0.00"), amount1.getValue());
		assertEquals(new BigDecimal("5000.00"), amount2.getValue());
	}
	
	@Test
	@DisplayName("正常系：マイナス金額への加算が正しく動作する")
	void testAdd_正常系_マイナス金額への加算() {
		// 準備
		BalanceTotalAmount amount1 = BalanceTotalAmount.from(new BigDecimal("-8000.00"));
		BalanceAmount amount2 = BalanceAmount.from(new BigDecimal("5000.00"));

		// 実行
		BalanceTotalAmount result = amount1.add(amount2);

		// 検証
		assertEquals(new BigDecimal("-3000.00"), result.getValue());
		// 元のオブジェクトは変更されていないことを確認（不変性）
		assertEquals(new BigDecimal("-8000.00"), amount1.getValue());
		assertEquals(new BigDecimal("5000.00"), amount2.getValue());
	}
	
	@Test
	@DisplayName("正常系：マイナス金額同士の加算が正しく動作する")
	void testAdd_正常系_マイナス金額どおしの加算() {
		// 準備
		BalanceTotalAmount amount1 = BalanceTotalAmount.from(new BigDecimal("-8000.00"));
		BalanceAmount amount2 = BalanceAmount.from(new BigDecimal("-5000.00"));

		// 実行
		BalanceTotalAmount result = amount1.add(amount2);

		// 検証
		assertEquals(new BigDecimal("-13000.00"), result.getValue());
		// 元のオブジェクトは変更されていないことを確認（不変性）
		assertEquals(new BigDecimal("-8000.00"), amount1.getValue());
		assertEquals(new BigDecimal("-5000.00"), amount2.getValue());
	}
	
	@Test
	@DisplayName("正常系：isDeficitが正しく動作する")
	void testIsDeficit() {
		// 準備
		BalanceTotalAmount deficit = BalanceTotalAmount.from(new BigDecimal("-1000.00"));
		BalanceTotalAmount surplus = BalanceTotalAmount.from(new BigDecimal("1000.00"));
		BalanceTotalAmount zero = BalanceTotalAmount.ZERO;

		// 検証
		assertTrue(deficit.isDeficit());
		assertFalse(surplus.isDeficit());
		assertFalse(zero.isDeficit());
	}

	@Test
	@DisplayName("正常系：isSurplusが正しく動作する")
	void testIsSurplus() {
		// 準備
		BalanceTotalAmount deficit = BalanceTotalAmount.from(new BigDecimal("-1000.00"));
		BalanceTotalAmount surplus = BalanceTotalAmount.from(new BigDecimal("1000.00"));
		BalanceTotalAmount zero = BalanceTotalAmount.ZERO;

		// 検証
		assertFalse(deficit.isSurplus());
		assertTrue(surplus.isSurplus());
		assertFalse(zero.isSurplus());
	}

}

...
class PaymentDateTest {
	...
	void testFrom_LocalDate_Null() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			...
			() -> PaymentDate.from((LocalDate) null)
		);
	...
	void testIsBefore() {
		// 準備
		PaymentDate date1 = PaymentDate.from("20251128");
		PaymentDate date2 = PaymentDate.from("20251129");
	...
	void testIsAfter() {
		// 準備
		PaymentDate date1 = PaymentDate.from("20251130");
		PaymentDate date2 = PaymentDate.from("20251129");
	...
	void testEquals_SameValue() {
		// 準備
		PaymentDate date1 = PaymentDate.from("20251129");
		PaymentDate date2 = PaymentDate.from("20251129");
	...
	void testBoundaryValue_EndOfMonth() {
		// 実行：2月末日（閏年）
		PaymentDate leapYear = PaymentDate.from("20240229");
		assertEquals(LocalDate.of(2024, 2, 29), leapYear.getValue());

		// 実行：2月末日（平年）
		PaymentDate normalYear = PaymentDate.from("20250228");
		assertEquals(LocalDate.of(2025, 2, 28), normalYear.getValue());

		// 実行：12月末日
		PaymentDate endOfYear = PaymentDate.from("20251231");
	...
	void testBoundaryValue_StartOfMonth() {
		// 実行
		PaymentDate result = PaymentDate.from("20251101");

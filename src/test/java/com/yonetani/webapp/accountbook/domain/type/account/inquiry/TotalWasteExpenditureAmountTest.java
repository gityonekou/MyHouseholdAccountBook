/**
 * TotalWasteExpenditureAmount(無駄遣い合計支出金額)のテストクラスです。
 *
 *------------------------------------------------
 * 更新履歴
 * 日付       : version  ブランチ            コメントなど
 * 2025/12/25 : 1.00.00  feature-1.00-dev00  新規作成
 * 2025/12/28 : 1.00.01  feature-1.00-dev00  クラス名変更(SisyutuKingakuBCTest → TotalWasteExpenditureAmountTest)
 *
 */
 ...
 *
 */
@DisplayName("無駄遣い合計支出金額(TotalWasteExpenditureAmount)のテスト")
class TotalWasteExpenditureAmountTest {
	...
	void testFrom_正常系_軽度と重度両方値あり() {
		...
		// 実行
		TotalWasteExpenditureAmount amount = TotalWasteExpenditureAmount.from(minor, severe);

		// 検証
		assertNotNull(amount);
		assertEquals(new BigDecimal("15000.00"), amount.getValue()); // 軽度 + 重度
		assertEquals(minor, amount.getMinorWasteExpenditureAmount());
		assertEquals(severe, amount.getSevereWasteExpenditureAmount());
	...
	void testZERO定数() {
		// 検証
		assertNotNull(TotalWasteExpenditureAmount.ZERO);
		assertEquals(BigDecimal.ZERO.setScale(2), TotalWasteExpenditureAmount.ZERO.getValue());
		assertTrue(TotalWasteExpenditureAmount.ZERO.getMinorWasteExpenditureAmount().isZero());
		assertTrue(TotalWasteExpenditureAmount.ZERO.getSevereWasteExpenditureAmount().isZero());
	}
	...
	void testFrom_異常系_軽度がnull() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			...
			() -> TotalWasteExpenditureAmount.from(null, severe)
		);
	...
	void testFrom_異常系_重度がnull() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> TotalWasteExpenditureAmount.from(minor, null)
		);
	...
	void testAdd_正常系() {
		// 準備
		TotalWasteExpenditureAmount amount1 = TotalWasteExpenditureAmount.from(
			MinorWasteExpenditureAmount.from(new BigDecimal("10000.00")),
			SevereWasteExpenditureAmount.from(new BigDecimal("5000.00"))
		);
		TotalWasteExpenditureAmount amount2 = TotalWasteExpenditureAmount.from(
		...
		assertEquals(new BigDecimal("13000.00"), result.getMinorWasteExpenditureAmount().getValue());
		assertEquals(new BigDecimal("7000.00"), result.getSevereWasteExpenditureAmount().getValue());
		// 元の値が変わっていないこと
	...
	void testGetPercentage() {
		// 準備
		TotalWasteExpenditureAmount amount = TotalWasteExpenditureAmount.from(
			MinorWasteExpenditureAmount.from(new BigDecimal("10000.00")),
			SevereWasteExpenditureAmount.from(new BigDecimal("5000.00"))
		);

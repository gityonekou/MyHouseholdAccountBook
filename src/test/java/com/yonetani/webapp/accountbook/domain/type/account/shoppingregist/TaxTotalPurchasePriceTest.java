/**
 * TaxTotalPurchasePrice(消費税合計)のテストクラスです。
 *
 *------------------------------------------------
 * 更新履歴
 ...
 *
 */
@DisplayName("消費税合計(TaxTotalPurchasePrice)のテスト")
class TaxTotalPurchasePriceTest {

	@Test
	@DisplayName("正常系：正の金額で生成できる")
	void testFrom_正常系_正の金額() {
		// 実行
		TaxTotalPurchasePrice amount = TaxTotalPurchasePrice.from(new BigDecimal("10000.00"));
	...
	void testFrom_正常系_null値() {
		// 実行
		TaxTotalPurchasePrice amount = TaxTotalPurchasePrice.from(null);
	...
	void testFrom_異常系_負の金額() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			...
			() -> TaxTotalPurchasePrice.from(new BigDecimal("-1000.00"))
		);
		assertTrue(exception.getMessage().contains("消費税合計"));
	...
	void testFrom_異常系_スケール値不正() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> TaxTotalPurchasePrice.from(new BigDecimal("10000"))
		);
		assertTrue(exception.getMessage().contains("消費税合計"));
	...
	void testAdd_正常系() {
		// 準備
		TaxTotalPurchasePrice amount1 = TaxTotalPurchasePrice.from(new BigDecimal("10000.00"));
		TaxTotalPurchasePrice amount2 = TaxTotalPurchasePrice.from(new BigDecimal("5000.00"));

		// 実行
		TaxTotalPurchasePrice result = amount1.add(amount2);
	...
	void testAdd_正常系_null値() {
		// 準備
		TaxTotalPurchasePrice amount1 = TaxTotalPurchasePrice.from(new BigDecimal("10000.00"));
		TaxTotalPurchasePrice amount2 = TaxTotalPurchasePrice.from(null);
	...
	void testEquals_正常系_値ベースで一致() {
		// 準備
		TaxTotalPurchasePrice amount1 = TaxTotalPurchasePrice.from(new BigDecimal("10000.00"));
		TaxTotalPurchasePrice amount2 = TaxTotalPurchasePrice.from(new BigDecimal("10000.00"));

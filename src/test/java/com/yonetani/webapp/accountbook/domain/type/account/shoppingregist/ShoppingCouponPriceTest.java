/**
 * ShoppingCouponPrice(買い物登録の「クーポン金額」項目)のテストクラスです。
 *
 *------------------------------------------------
 * 更新履歴
 * 日付       : version  ブランチ            コメントなど
 * 2026/09/21 : 1.00.00  feature-1.03-dev1   新規作成
 *
 */
 ...
 *
 */
@DisplayName("買い物登録の「クーポン金額」項目(ShoppingCouponPrice)のテスト")
class ShoppingCouponPriceTest {
	@Test
	@DisplayName("正常系：正の金額で生成できる")
	void testFrom_正常系_正の金額() {
		// 実行
		ShoppingCouponPrice amount = ShoppingCouponPrice.from(new BigDecimal("10000.00"));
	...
	void testFrom_正常系_null値() {
		// 実行
		ShoppingCouponPrice amount = ShoppingCouponPrice.from((BigDecimal)null);
	...
	void testFrom_異常系_負の金額() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			...
			() -> ShoppingCouponPrice.from(new BigDecimal("-1000.00"))
		);
	...
	void testFrom_異常系_スケール値不正() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> ShoppingCouponPrice.from(new BigDecimal("10000"))
		);
		assertTrue(exception.getMessage().contains("クーポン金額"));
	...
	void testFromInteger_正常系_正の金額() {
		// 実行
		ShoppingCouponPrice amount = ShoppingCouponPrice.from(Integer.valueOf(10000));
	...
	void testFromInteger_正常系_ゼロ() {
		// 実行
		ShoppingCouponPrice amount = ShoppingCouponPrice.from(Integer.valueOf(0));
	...
	void testFromInteger_正常系_null値() {
		// 実行
		ShoppingCouponPrice amount = ShoppingCouponPrice.from((Integer)null);
	...
	void testFromInteger_異常系_負の金額() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> ShoppingCouponPrice.from(Integer.valueOf(-1000))
		);
		assertTrue(exception.getMessage().contains("クーポン金額"));
		assertTrue(exception.getMessage().contains("マイナス"));
	}
	...
	void testNULL定数() {
		...
		// 準備
		ShoppingCouponPrice nullValue = ShoppingCouponPrice.from((BigDecimal)null);
		
		// 検証
		assertNotNull(ShoppingCouponPrice.NULL);
		assertNull(ShoppingCouponPrice.NULL.getValue());
		assertEquals(ShoppingCouponPrice.NULL, nullValue);
	}
	...
	void testAdd_正常系() {
		// 準備
		ShoppingCouponPrice amount1 = ShoppingCouponPrice.from(new BigDecimal("10000.00"));
		ShoppingCouponPrice amount2 = ShoppingCouponPrice.from(new BigDecimal("5000.00"));

		// 実行
		ShoppingCouponPrice result = amount1.add(amount2);
	...
	void testAdd_正常系_null値() {
		// 準備
		ShoppingCouponPrice amount1 = ShoppingCouponPrice.from(new BigDecimal("10000.00"));
		ShoppingCouponPrice amount2 = ShoppingCouponPrice.from((BigDecimal)null);

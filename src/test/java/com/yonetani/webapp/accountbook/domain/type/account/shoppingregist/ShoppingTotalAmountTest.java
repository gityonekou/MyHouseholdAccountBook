/**
 * ShoppingTotalAmount(買い物合計金額)のテストクラスです。
 *
 *------------------------------------------------
 * 更新履歴
 ...
 *
 */
@DisplayName("買い物合計金額(ShoppingTotalAmount)のテスト")
class ShoppingTotalAmountTest {

	@Test
	@DisplayName("正常系：正の金額で生成できる")
	void testFrom_正常系_正の金額() {
		// 実行
		ShoppingTotalAmount amount = ShoppingTotalAmount.from(new BigDecimal("10000.00"));
	...
	void testZERO定数() {
		// 検証
		assertNotNull(ShoppingTotalAmount.ZERO);
		assertTrue(ShoppingTotalAmount.ZERO.isZero());
		assertEquals(BigDecimal.ZERO.setScale(2), ShoppingTotalAmount.ZERO.getValue());
	}
	...
	void testFrom_異常系_null値() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			...
			() -> ShoppingTotalAmount.from(null)
		);
		assertTrue(exception.getMessage().contains("買い物合計金額"));
	...
	void testFrom_異常系_負の金額() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> ShoppingTotalAmount.from(new BigDecimal("-1000.00"))
		);
		assertTrue(exception.getMessage().contains("買い物合計金額"));
	...
	void testFrom_異常系_スケール値不正() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> ShoppingTotalAmount.from(new BigDecimal("10000"))
		);
		assertTrue(exception.getMessage().contains("買い物合計金額"));
	...
	void testAdd_正常系() {
		// 準備
		ShoppingTotalAmount amount1 = ShoppingTotalAmount.from(new BigDecimal("10000.00"));
		ShoppingTotalAmount amount2 = ShoppingTotalAmount.from(new BigDecimal("5000.00"));

		// 実行
		ShoppingTotalAmount result = amount1.add(amount2);
	...
	void testEquals_正常系_値ベースで一致() {
		// 準備
		ShoppingTotalAmount amount1 = ShoppingTotalAmount.from(new BigDecimal("10000.00"));
		ShoppingTotalAmount amount2 = ShoppingTotalAmount.from(new BigDecimal("10000.00"));

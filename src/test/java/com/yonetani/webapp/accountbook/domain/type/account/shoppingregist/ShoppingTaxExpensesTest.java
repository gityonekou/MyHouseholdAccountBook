/**
 * ShoppingTaxExpenses(買い物カテゴリごとの消費税)のテストクラスです。
 *
 *------------------------------------------------
 * 更新履歴
 ...
 *
 */
@DisplayName("買い物カテゴリごとの消費税(ShoppingTaxExpenses)のテスト")
class ShoppingTaxExpensesTest {

	@Test
	@DisplayName("正常系：正の金額で生成できる")
	void testFrom_正常系_正の金額() {
		// 実行
		ShoppingTaxExpenses amount = ShoppingTaxExpenses.from(new BigDecimal("10000.00"));
	...
	void testFrom_正常系_null値() {
		// 実行
		ShoppingTaxExpenses amount = ShoppingTaxExpenses.from((BigDecimal)null);
	...
	void testFrom_異常系_負の金額() {
		...
		assertTrue(exception.getMessage().contains("消費税金額"));
	...
	void testFrom_異常系_スケール値不正() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> ShoppingTaxExpenses.from(new BigDecimal("10000"))
		);
		assertTrue(exception.getMessage().contains("消費税金額"));
	...
	void testFromInteger_正常系_正の金額() {
		// 実行
		ShoppingTaxExpenses amount = ShoppingTaxExpenses.from(Integer.valueOf(10000));
	...
	void testFromInteger_正常系_ゼロ() {
		// 実行
		ShoppingTaxExpenses amount = ShoppingTaxExpenses.from(Integer.valueOf(0));
	...
	void testFromInteger_正常系_null値() {
		// 実行
		ShoppingTaxExpenses amount = ShoppingTaxExpenses.from((Integer)null);
	...
	void testFromInteger_異常系_負の金額() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> ShoppingTaxExpenses.from(Integer.valueOf(-1000))
		);
		assertTrue(exception.getMessage().contains("消費税金額"));
		assertTrue(exception.getMessage().contains("マイナス"));
	}
	...
	void testNULL定数() {
		...
		// 準備
		ShoppingTaxExpenses nullValue = ShoppingTaxExpenses.from((BigDecimal)null);
		
		// 検証
		assertNotNull(ShoppingTaxExpenses.NULL);
		assertNull(ShoppingTaxExpenses.NULL.getValue());
		assertEquals(ShoppingTaxExpenses.NULL, nullValue);
	}
	...
	void testAdd_正常系() {
		// 準備
		ShoppingTaxExpenses amount1 = ShoppingTaxExpenses.from(new BigDecimal("10000.00"));
		ShoppingTaxExpenses amount2 = ShoppingTaxExpenses.from(new BigDecimal("5000.00"));

		// 実行
		ShoppingTaxExpenses result = amount1.add(amount2);
	...
	void testAdd_正常系_null値() {
		// 準備
		ShoppingTaxExpenses amount1 = ShoppingTaxExpenses.from(new BigDecimal("10000.00"));
		ShoppingTaxExpenses amount2 = ShoppingTaxExpenses.from((BigDecimal)null);

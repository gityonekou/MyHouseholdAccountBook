/**
 * SyuunyuuKingaku(収入金額)のテストクラスです。
 *
 *------------------------------------------------
 * 更新履歴
 * 日付       : version  コメントなど
 ...
 *
 */
@DisplayName("収入金額(SyuunyuuKingaku)のテスト")
class SyuunyuuKingakuTest {

	@Test
	@DisplayName("正常系：正の金額で生成できる")
	void testFrom_正常系_正の金額() {
		// 実行
		SyuunyuuKingaku amount = SyuunyuuKingaku.from(new BigDecimal("10000.00"));
	...
	void testFrom_正常系_0円() {
		// 実行
		SyuunyuuKingaku amount = SyuunyuuKingaku.from(BigDecimal.ZERO.setScale(2));
	...
	void testZERO定数() {
		// 検証
		assertNotNull(SyuunyuuKingaku.ZERO);
		assertTrue(SyuunyuuKingaku.ZERO.isZero());
		assertEquals(BigDecimal.ZERO.setScale(2), SyuunyuuKingaku.ZERO.getValue());
	}
	...
	void testFrom_異常系_null値() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			...
			() -> SyuunyuuKingaku.from((BigDecimal)null)
		);
	...
	void testFrom_異常系_負の金額() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> SyuunyuuKingaku.from(new BigDecimal("-1000.00"))
		);
		assertTrue(exception.getMessage().contains("収入金額"));
	...
	void testFrom_異常系_スケール値不正() {
		...
		MyHouseholdAccountBookRuntimeException exception = assertThrows(
			MyHouseholdAccountBookRuntimeException.class,
			() -> SyuunyuuKingaku.from(new BigDecimal("10000"))
		);
		assertTrue(exception.getMessage().contains("収入金額"));
	...
	void testCompareTo_正常系_大きい() {
		// 準備
		SyuunyuuKingaku amount1 = SyuunyuuKingaku.from(new BigDecimal("10000.00"));
		SyuunyuuKingaku amount2 = SyuunyuuKingaku.from(new BigDecimal("5000.00"));
	...
	void testCompareTo_正常系_等しい() {
		// 準備
		SyuunyuuKingaku amount1 = SyuunyuuKingaku.from(new BigDecimal("10000.00"));
		SyuunyuuKingaku amount2 = SyuunyuuKingaku.from(new BigDecimal("10000.00"));
	...
	void testCompareTo_正常系_小さい() {
		// 準備
		SyuunyuuKingaku amount1 = SyuunyuuKingaku.from(new BigDecimal("5000.00"));
		SyuunyuuKingaku amount2 = SyuunyuuKingaku.from(new BigDecimal("10000.00"));
	...
	void testIsZero() {
		// 検証
		assertTrue(SyuunyuuKingaku.ZERO.isZero());
		assertFalse(SyuunyuuKingaku.from(new BigDecimal("1.00")).isZero());
	}
	...
	void testIsPositive() {
		// 検証
		assertFalse(SyuunyuuKingaku.ZERO.isPositive());
		assertTrue(SyuunyuuKingaku.from(new BigDecimal("1.00")).isPositive());
	}

	@Test
	@DisplayName("正常系：isNegativeが正しく動作する（収入金額は常にfalse）")
	void testIsNegative() {
		// 検証（収入金額は負の値を持てないため、常にfalse）
		assertFalse(SyuunyuuKingaku.ZERO.isNegative());
		assertFalse(SyuunyuuKingaku.from(new BigDecimal("1.00")).isNegative());
	}
	...
	void testEquals() {
		...
		SyuunyuuKingaku amount1 = SyuunyuuKingaku.from(new BigDecimal("10000.00"));
		SyuunyuuKingaku amount2 = SyuunyuuKingaku.from(new BigDecimal("10000.00"));
		SyuunyuuKingaku amount3 = SyuunyuuKingaku.from(new BigDecimal("5000.00"));
	...
	void testToString() {
		// 準備
		SyuunyuuKingaku amount = SyuunyuuKingaku.from(new BigDecimal("12345.67"));
	...
	void testToFormatString() {
		// 準備
		SyuunyuuKingaku amount1 = SyuunyuuKingaku.from(new BigDecimal("12345.67"));
		SyuunyuuKingaku amount2 = SyuunyuuKingaku.from(new BigDecimal("1000000.00"));
	...
	void testToIntegerValue() {
		...
		SyuunyuuKingaku amount1 = SyuunyuuKingaku.from(new BigDecimal("12345.67"));
		SyuunyuuKingaku amount2 = SyuunyuuKingaku.from(new BigDecimal("1000000.00"));
		SyuunyuuKingaku amount3 = SyuunyuuKingaku.from(new BigDecimal("12345.44"));
		SyuunyuuKingaku amount4 = SyuunyuuKingaku.from(new BigDecimal("12345.50"));

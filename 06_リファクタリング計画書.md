...
**注記 - Phase 1における適用範囲**:
- Phase 1の新規クラス作成（値オブジェクト基底クラスなど）は、実装後のテスト作成でも可
- ただし、**テストカバレッジ90%以上**を必ず確保すること
- Phase 2以降の既存機能リファクタリングでは、テストファースト必須
- 複雑なビジネスロジック（ドメインサービス、集約）は、テストファースト推奨

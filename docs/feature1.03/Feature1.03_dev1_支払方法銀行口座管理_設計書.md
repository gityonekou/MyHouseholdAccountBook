...
（`ExpenditureItemCode`/`ExpenditureCategory`の8組の実際の値は `ShoppingRegistExpenditureItemComponent` 既存コードの定数定義と完全に一致させること）

> **【`【更新不可予定額(アプリ更新)】` への依存は存在しない・突合レビュー指摘S2】** 設計指針2.6・要件定義書3.3/7.3は「項目名に `【更新不可予定額(アプリ更新)】` が付くもの」という文字列ベースの説明を条件付きで残していたが、**dev1着手前のソースを `アプリ更新` / `更新不可予定額` でgrepした結果、いずれの文字列もコード中に存在しないことが確認された**（ユーザー確認済み）。したがって「文字列依存を本Specificationに置き換える」という作業は発生せず、本Specificationは最初から `(支出項目コード, 支出区分)` のみで判定する。
>
> `【更新不可予定額(アプリ更新)】` は、**ユーザーが固定費を登録する際の命名規約**（この固定費の予定額はアプリが自動更新するため手動更新しない、という運用上の目印）であり、`FIXED_COST_NAME` に含まれる。収支登録処理で `EXPENDITURE_NAME` にそのまま引き継がれる。コード上のいかなる判定にも使われていないため、本Specificationにも移行スクリプトの判定条件にも取り込まない（6.1節参照）。
>
> **区別すべき点**：文字列依存が存在しないのは `【更新不可予定額(アプリ更新)】` についてのみである。支出区分の導出に使う `無駄遣いB` / `無駄遣いC`（`ExpenditureCategory.from(FixedCostName)`）への依存は実在し、dev1でも利用する（突合レビュー指摘W・AC。10章備考のdev5申し送り参照）。
...
2. `EXPENDITURE_TABLE` のうち、`(SISYUTU_ITEM_CODE, EXPENDITURE_KUBUN)` の組み合わせが `ShoppingAggregateSpecification`（3.9節）の対象8項目に一致する行 → 「支払方法がない」固定値を設定。判定条件はSpecificationと完全に一致させ、別途SQL側で独自の条件を書かない。
   - **抽出件数の検証を移行ロジックに組み込む（突合レビュー指摘S1）**：`EXPENDITURE_TABLE` のPKは `(USER_ID, TARGET_YEAR, TARGET_MONTH, EXPENDITURE_CODE)` であり、`SISYUTU_ITEM_CODE` に対する一意制約は存在しない（`schema.sql`）。8項目の重複登録はアプリ側で登録時に例外として弾いているため実データに重複行は存在しない前提だが、**この重複チェックが実装される以前に登録された行が本番に残っている可能性は排除できない**。移行は本番データに一度だけ当てる操作であるため、UPDATE前に「ユーザー×対象年月あたりの抽出件数が8件であること」を検証するロジックを移行スクリプトに組み込み、8件でない場合は移行を中断して個別確認する（ユーザー確認により本方針を採用）。
   - **照合材料としての `EXPENDITURE_NAME`（突合レビュー指摘S2）**：対象8項目の `EXPENDITURE_NAME` は、引き継ぎ元の `FIXED_COST_NAME` を継承しており必ず `【更新不可予定額(アプリ更新)】` を含む（支出区分がB/Cの行はさらに `【無駄遣いB】`／`【無駄遣いC】` の接頭辞が付く）。上記の件数検証で差異が出た場合、この文字列で抽出した結果と突き合わせればどの行が想定外かを特定しやすい。**ただしこれは目視・調査用の照合材料に限り、UPDATE文の判定条件には一切入れない**。コード上に存在しない文字列依存を移行スクリプトが新たに作ることになり、設計指針2.6の方針に逆行するため（3.9節の注記参照）。
3. **`FIXED_COST_TABLE` のうち、対象8項目に対応する固定費行 → 「支払方法がない」固定値を設定**（突合レビュー指摘T対応）。改訂5で判明したとおり、この8項目は固定費として登録され「固定費→収支登録」の引き継ぎ機能で毎月`EXPENDITURE_TABLE`へ伝播する運用のため、既存の`EXPENDITURE_TABLE`行だけを2.で変換しても、参照元の`FIXED_COST_TABLE`行を変換し忘れると、**翌月の収支登録初期表示時に固定費側の値（他ステップで設定された値）が引き継がれてしまい、移行直後の月と翌月以降とで支払方法が食い違うサイレントな不整合**が生じる。
   - `FIXED_COST_TABLE` には`ShoppingAggregateSpecification`が要求する支出区分（無駄遣い区分）に相当する専用カラムが存在しない（指摘W参照）。区分の判定は既存の`ExpenditureCategory.from(FixedCostName)`（`domain/type/account/expenditure/ExpenditureCategory.java:94-102`。Feature1.03以前から存在する固定費名の文字列マッチによる支出区分導出ロジック）と同じ規則に基づくが、**この文字列マッチ規則をSQL側（UPDATE文のWHERE句）でそのまま再現することはしない**（突合レビュー指摘AB対応）。移行対象は全ユーザー合計でも高々8項目×ユーザー数程度の少件数であり、文字列マッチをSQLに複製すると、`ExpenditureCategory`側のロジック変更時にSQLの改修漏れが生じるリスクがある。代わりに以下の**候補抽出→目視確認→コード明示列挙**の手順を取る：
     ...
     2. 抽出結果を**目視で確認**し、`FIXED_COST_NAME`から対象8項目（支出項目コード＋無駄遣い区分の組み合わせ）に該当する行であることを1件ずつ確定させる。抽出件数が要件7.3の8項目（ユーザーごと）と一致することを確認し、想定外の固定費が混入していないかもあわせて確認する。対象8項目の`FIXED_COST_NAME`は運用上必ず`【更新不可予定額(アプリ更新)】`を含む（突合レビュー指摘S2。下記参照）ため、この文字列の有無を目視確認の手がかりに使ってよい。ただし**3.のUPDATE文の条件には入れない**（本節冒頭の方針どおり、確定した`FIXED_COST_CODE`の明示列挙のみを使う）。
   ...
   - **対象8項目の命名規約（突合レビュー指摘S2・ユーザー確認済み）**：8項目の固定費は、ユーザーが「この固定費の予定額はアプリが自動更新するため手動更新しない」ことを示す目印として、`FIXED_COST_NAME` に `【更新不可予定額(アプリ更新)】` を付ける運用になっている。この名称は収支登録処理で `EXPENDITURE_NAME` にそのまま引き継がれる（支出区分がB/Cの行は、`ExpenditureCategory` の導出結果に基づき `【無駄遣いB】`／`【無駄遣いC】` の接頭辞がさらに付与される）。実データの例：

     | `FIXED_COST_NAME`（固定費名） | `EXPENDITURE_NAME`（支出名） |
     |---|---|
     | 衣類・クリーニング・靴【更新不可予定額(アプリ更新)】 | 衣類・クリーニング・靴【更新不可予定額(アプリ更新)】 |
     | 住居設備【更新不可予定額(アプリ更新)】 | 住居設備【更新不可予定額(アプリ更新)】 |
     | 日用消耗品【更新不可予定額(アプリ更新)】 | 日用消耗品【更新不可予定額(アプリ更新)】 |
     | 食費：無駄遣いなし【更新不可予定額(アプリ更新)】 | 食費：無駄遣いなし【更新不可予定額(アプリ更新)】 |
     | 食費：無駄遣いB【更新不可予定額(アプリ更新)】 | 【無駄遣いB】食費：無駄遣いB【更新不可予定額(アプリ更新)】 |
     | 食費：無駄遣いC【更新不可予定額(アプリ更新)】 | 【無駄遣いC】食費：無駄遣いC【更新不可予定額(アプリ更新)】 |
     | 外食【更新不可予定額(アプリ更新)】 | 外食【更新不可予定額(アプリ更新)】 |
     | 仕事(流動経費)【更新不可予定額(アプリ更新)】 | 仕事(流動経費)【更新不可予定額(アプリ更新)】 |

     この文字列は**コード上のいかなる判定にも使われていない**（dev1着手前のソースを`アプリ更新`/`更新不可予定額`でgrepして不検出を確認済み。3.9節参照）。移行スクリプトでも判定条件には使わず、目視確認の照合材料としてのみ利用する。
...
- **【疎通確認指摘・改訂12】支払方法マスタに「支払方法メモ」項目を追加**：`PAYMENT_METHOD_TABLE`に`PAYMENT_METHOD_MEMO VARCHAR(400)`（null可）を`PAYMENT_METHOD_NAME`と`PAYMENT_METHOD_KUBUN`の間に追加（`PaymentMethodMemo`ドメインタイプ、`BankAccountMemo`と同じパターンで新設）。支払方法マスタ管理画面にtextarea入力欄として実装済み（一覧表示は対象外）。**この項目はdev2の「クレジットカードポイント減算」機能で、設定値がある場合のみ画面表示する用途を想定**しており、dev2設計開始時に紙芝居（クレカポイント減算画面）の再修正が必要（現状の紙芝居より入力項目を増やす方針）。

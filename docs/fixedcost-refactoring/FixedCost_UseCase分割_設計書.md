...
|---|---|---|---|
| `FixedCostInfoManageUseCaseIntegrationTest` | 12 | 参照系UseCase全メソッド | `FixedCostInfoManageIntegrationTest.sql` |
...
| `0004` | その他任意テスト | 0038（ガス代） | その他任意（40） | 27日 | 10,000円 | 支払月任意コンテキスト確認用（`readActSelectItemInfo`・`readUpdateFixedCostInfo` テスト対象） |
...
4位: 0004（その他任意テスト ← sort: 0306020000）
```

0004は0002の直後に来る（ガス代 0306020000 > 電気代 0306010000）ため、既存の1〜3位の順序は変わらない。
...
TUKI='40'（その他任意）は毎月（TUKI='00'）と同じ扱いで奇数月・偶数月の両合計に加算される。

```
奇数月合計: 60,000(0001:毎月) + 12,000(0002:毎月) + 16,590(0003:奇数月) + 10,000(0004:その他任意) = 98,590円
偶数月合計: 60,000(0001:毎月) + 12,000(0002:毎月) + 10,000(0004:その他任意) = 82,000円
```
...
| # | テスト名 | 対象メソッド | 検証内容 |
|---|---|---|---|
| ⓪ | 固定費0件_支出項目0件（未登録ユーザ） | `readInitInfo` | リスト0件、合計null、支出項目0件、0件メッセージ両方あり |
| ① | 固定費4件一覧表示・支出項目Level1 6件 | `readInitInfo` | リスト4件・DB順序、合計金額（奇数月98,590円・偶数月82,000円）、支出項目Level1 6件コード確認 |
| ② | 処理選択画面_家賃(0001) | `readActSelectItemInfo` | 固定費詳細情報・支出項目名「固定費(課税)＞地代家賃＞家賃」、固定費一覧4件 |
| ②′ | 処理選択画面_その他任意(0004) | `readActSelectItemInfo` | `shiharaiTukiDetailContext`=「その他任意(不定期の支払です)」・支出項目名「固定費(課税)＞水光熱通費＞ガス代」 |
...
| ⑧ | 更新画面_固定費0004（支払月任意詳細含む） | `readUpdateFixedCostInfo` | フォーム全フィールド確認（code/name/kubun/tuki=40/optionalContext=「不定期の支払です」/day/kingaku） |
...
| # | テスト名 | 対象メソッド | 検証内容 |
|---|---|---|---|
| ① | 固定費0003論理削除 | `execDelete` | トランザクション完了、DELETE_FLG=true、他レコード影響なし、完了メッセージ完全一致確認 |
| ② | 存在しない固定費コードで例外 | `execDelete` | `MyHouseholdAccountBookRuntimeException` 発生 |
| ③ | 新規固定費追加_0035 | `execAdd` | トランザクション完了、DB5件（追加前4件→追加後5件）、追加レコードの固定費コード（0005）・全フィールド確認、完了メッセージ完全一致確認 |
| ⑤ | 固定費0002更新 | `execUpdate` | トランザクション完了、固定費名・支払金額更新確認、件数変化なし（4件）、完了メッセージ完全一致確認 |
| ⑥ | 存在しない固定費コードで更新例外 | `execUpdate` | `MyHouseholdAccountBookRuntimeException` 発生 |

> **完了メッセージ検証方針**: `assertTrue(contains(...))` ではなく `assertEquals` で完全一致確認する。
> メッセージ形式は `"[処理内容]。[code:NNNN]支払名"` （例: `"指定の固定費を削除しました。[code:0003]国民年金保険"`）。

### 7.3 FixedCostInfoManageControllerIntegrationTest

| # | エンドポイント | 検証内容 |
|---|---|---|
| 1 | `GET /initload/` | view名・fixedCostItemList 4件・loginUserName |
| 2 | `GET /select?fixedCostCode=0001` | view名・fixedCostInfo・fixedCostItemList 4件 |
...
| 7 | `POST /updateload/ (actionCancel)` | view名(Init)・fixedCostItemList 4件 |
...
| 12 | `GET /updateComplete/` | view名(Init)・fixedCostItemList 4件 |
...
| `AbstractFixedCostItemListResponse.java` | `fixedCostItemList`・`oddMonthGoukei`・`anEvenMonthGoukei` に `@Getter` 追加 |
| `FixedCostInfoManageActSelectResponse.java` | `fixedCostInfo` に `@Getter` 追加 |
| `FixedCostInfoManageInitResponse.java` | `registeredFlg`・`sisyutuItemCodeInfo`・`registeredFixedCostInfoList` に `@Getter` 追加 |
| `FixedCostInfoManageUpdateResponse.java` | `fixedCostInfoUpdateForm`・`fixedCostKubunSelectList`・`shiharaiTukiSelectList`・`shiharaiDaySelectList`・`sisyutuItemName` に `@Getter` 追加 |
| `docs/integration-test-guidelines.md` | UseCase統合テストにおける ModelMap 使用禁止ルールを追記（§3.1） |
...
| `FixedCostInfoManageUseCaseIntegrationTest.java` | 参照系UseCaseの統合テスト（12件） |
...
```
Tests run: 837, Failures: 0, Errors: 0, Skipped: 0
BUILD SUCCESS
```
